...
def initial_setup(db: Session = Depends(get_db)):
    ...
    try:
        ...
        existing_merchant = db.query(User).filter(User.email == "merchant@lumina.com").first()
        if not existing_merchant:
            ...
            db.add(merchant)
            created_items.append("Merchant User")
        ...
        if db.query(Category).count() == 0:
            ...
            created_items.append(f"{len(db_categories)} Categories")
        else:
            db_categories = db.query(Category).all()
        
        # Check if products exist
        if db.query(Product).count() == 0 and db_categories:
            ...
            products_data = [
                ...
                {"name": "PlayStation 5", "brand": "Sony", "price": 499.99, "category_idx": 4,
                 ...
                 "specifications": {"Storage": "825GB SSD", "Resolution": "4K 120Hz", "Drive": "Ultra HD Blu-ray"}},
                {"name": "Atomic Habits", "brand": "James Clear", "price": 14.99, "category_idx": 4,
                 "description": "An Easy & Proven Way to Build Good Habits & Break Bad Ones.",
                 "image_url": "https://images.unsplash.com/photo-1544947950-fa07a98d237f?w=400",
                 "specifications": {"Format": "Hardcover", "Pages": "320", "Publisher": "Avery"},
                 "tags": ["bestseller"]},
                {"name": "Catan Board Game", "brand": "Catan Studio", "price": 44.00, "category_idx": 4,
                 "description": "Trade, build and settle the Island of Catan in this modern classic.",
                 "image_url": "https://images.unsplash.com/photo-1605901309584-818e25960b8f?w=400",
                 "specifications": {"Players": "3-4", "Time": "60 mins", "Age": "10+"}},
                {"name": "Fujifilm Instax Mini 12", "brand": "Fujifilm", "price": 79.95, "category_idx": 4,
                 "description": "Capture bright photos anytime, anywhere with automatic exposure.",
                 "image_url": "https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?w=400",
                 "specifications": {"Film": "Instax Mini", "Lens": "60mm", "Flash": "Automatic"}}
            ]
            
            for prod_data in products_data:
                # Get category
                cat_idx = prod_data.pop("category_idx")
                if 0 <= cat_idx < len(db_categories):
                    # Add merchant_id (use admin or merchant)
                    prod_data["merchant_id"] = merchant.id if merchant else admin.id
                    prod_data["category_id"] = db_categories[cat_idx].id
                    
                    product = Product(**prod_data)
                    db.add(product)
                    created_items.append(f"Product: {prod_data['name']}")
                
        db.commit()
        
        return {"message": "Setup completed successfully", "created": created_items}
